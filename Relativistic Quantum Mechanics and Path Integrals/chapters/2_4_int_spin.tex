...
\begin{enumerate}
    ...
    \item The second equation (the transversality condition) eliminates in a covariant way non-physical degrees of freedom: choosing the frame of reference at rest with the particle, where the 4-momentum reduces to \(\overline{p}^{\mu} = (m,\,0,\,0,\,0)\), one recognizes that the independent components of the wave function must have only spatial indices:
          ...
          \]
          where the index \(\mu = (0,i)\) is split into time and space components. This is due to the fact that when contracting with the 4-momentum at rest, only the time component survives, and so the equation imposes that all components with at least one time index vanish.
...
If we expand the first term, we find
\[
    \begin{aligned}
    ...
    \end{aligned}
\]
and now we can integrate by parts the second term and neglect boundary terms two times:
\[
...
\begin{equation}
    ...
    \label{eq:proca_motion_equations}
\end{equation}
...
\[
    \partial^{\mu} A_\mu = 0.
\]
Using this relationship, one rewrites Proca equations found estremizing the lagrangian in \eqref{eq:proca_motion_equations} as four Klein-Gordon equations plus the constraint, as in eq. \eqref{eq:proca_equations}:
\[
    \begin{aligned}
...
\]
so that integrating by parts and neglecting boundary terms one finds the equations of motion\footnote{For the complete computation see the previous section and consider \(m=0\).}
\[
    \partial^{\mu} F_{\mu \nu} = 0.
\]
...
\]
with \(\alpha(x)\) taken now as an infinitesimal arbitrary function. We can think of this local symmetry as associated with the \(\mathrm{U}(1)\) group, since one can write \eqref{eq:gauge_transformation} in the form
\[
