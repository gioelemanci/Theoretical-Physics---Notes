...
\]
where we have defined the traceless symmetric tensor \(\hat{S}^{ab}\) (which satisfies \(\hat{S}^a_{\ a} = 0\)).\footnote{We have divided by \(N\) since the trace involves summing over \(N\) components, and for the delta in particular, \(\delta^a_{\ a} = N\), since it is the trace of the identity \(N\times N\) matrix.} Thus, collecting all pieces, we have succeeded in separating the tensor \(T^{ab}\) into its irreducible parts:
\[
...
\[
    ...
    \end{aligned}
\]
where we used \(U^b_{\ a} \sim U_{\dot{b}}^{\ \dot{a}} = (U^a_{\ b})^\dagger\), thus we have seen that this representation preserves the scalar product among daggered and undaggered vectors.
...
\begin{example}[\(\mathrm{SU}(2)\) case and quantum mechanics]
    ...
    \end{equation}
    where \(2\) is the fundamental representation (acting on vectors \(v^a \sim v_{\dot{a}}\)), \(\bar{2}\) is the antifundamental representation (acting on vectors \(v^{\dot{a}} \sim v_a\)), \(1\) is the scalar representation, and \(3\) is the adjoint representation (acting on traceless tensors \(T^a_{\ b}\)).
    ...
    \]
    which indicates that, up to a change of basis given by the \(\epsilon_{ab}\) tensor, the vectors \(v^a\) and \(w_a\) transform in the same way. Here, we used the fact that \(\epsilon_{ab}\) is an invariant tensor. The explicit proof is as follows: if \(U \in SU(2)\) then:
    \[
        \epsilon'^{ab} = U^a_{\ c} U^b_{\ d} \epsilon^{cd} = k \epsilon^{ab}, \quad \epsilon^{ab} = \begin{pmatrix} 0 & 1 \\ -1 & 0 \end{pmatrix},
    \]
    ...
    \]
    where we constructed the determinant of the matrix \(U\) using the Levi-Civita symbol \(\epsilon^{cd}\). Comparing with \(\epsilon'^{12} = k \epsilon^{12}\), we find \(k = 1\) and \(\epsilon'^{ab} = \epsilon^{ab}\), and thus \(w_a\) transforms in the same way as \(v^a\) (up to a change of basis), confirming that \(\bar{2} \sim 2\).
...
\begin{example}[\(\mathrm{SU}(3)\) case and quark model]
    ...
    \end{equation}
    where \(3\) is the fundamental representation (acting on vectors \(v^a \sim v_{\dot{a}}\)), \(\bar{3}\) is the antifundamental representation (acting on vectors \(v^{\dot{a}} \sim v_a\)), \(1\) is the scalar representation, and \(8\) is the adjoint representation (acting on traceless tensors \(T^a_{\ b}\)).
