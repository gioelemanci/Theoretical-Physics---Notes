...
\begin{equation}
    \int \d{t} \frac{\delta J(t)}{\delta J(t_i)} x(t) = \int \d{t} \delta(t - t_i) x(t) = x(t_i).
...
\subsection{Digression on Gaussian Integrals}\label{sec:gaussian_integrals}
...
\begin{equation}
    Z[J] = \int \mathrm{D} \phi \, e^{\frac{i}{\hbar} \left( S[\phi] + J_i \phi^i \right)},
...
\[
    W[J] = \frac{1}{2} J_i G^{ij} J_j - \Lambda, \quad \Lambda = -\frac{i}{2} \ln(\det(K_{ij})) = -\frac{i}{2}\Tr (\ln(K_{ij})),
\]
...
\[
    S[x] = -\frac{1}{2} \int_{-\infty}^{\infty} \d{t} \, x(t) \left(\frac{\mathrm{d}^2}{\mathrm{d} t^2} + \omega^2 \right) x(t) = -\frac{1}{2}\int \int \d{t} \d{t^{\prime}} \, x(t) K(t, t^{\prime}) x(t^{\prime}),
\]
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.55\textwidth}
        ...
        Having computed the Green's function, we can write the generating functional in \eqref{eq:generating_functional_free_theory} and complete the square (reducing to the form in \eqref{eq:QM_gaussian_integral}) to obtain an explicit expression for \(Z[J]\), which will let us compute all correlation functions of the harmonic oscillator.
    \end{minipage}
\end{figure}
...
\]
where \(N = \det(K_{ij})^{-\frac{1}{2}} = Z[0]\) is a normalization constant. The inverse of the kinetic operator \(K\) is the Green's function \(G(x,y)\) of the Klein-Gordon operator.
...
\[
    \begin{aligned}
        \langle \phi(x) \phi(y) \rangle & = -i G(x-y) = -i \int \frac{\mathrm{d}^4 p}{(2\pi)^4} \, \frac{e^{ip_\mu (x^{\mu} - y^{\mu})}}{p^2 + m^2 - i \epsilon} =  i \int \frac{\mathrm{d}^4 p}{(2\pi)^4} \, \frac{e^{ip_\mu (x^{\mu} - y^{\mu})}}{(p^0)^2 - (p^i)^2 - m^2 + i \epsilon} \\
    ...
    \end{aligned}
\]
Here again the Feynman prescription is crucial to obtain the correct time-ordering in the two-point correlation function as \(\epsilon \sim \epsilon^{\prime} \to 0\), and by comparing this result with the harmonic oscillator case one sees that the Klein-Gordon field theory is indeed an infinite collection of harmonic oscillators, one for each momentum mode \(\mathbf{p}\), with frequency \(\omega = E_p = \sqrt{\mathbf{p}^2 + m^2}\).
