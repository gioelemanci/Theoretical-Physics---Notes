    ...
    \begin{minipage}{0.55\textwidth}
        \resizebox{\textwidth}{!}{
            \begin{tikzpicture}[
                ...
                % CFT -> QFT
                \draw[arrow] (CFT.east) -- (QFT.north) node[label, yshift=0.1cm] {II Quantization}; % Modificato yshift

                % Path 2: Classical Mechanics -> Quantum Mechanics -> QFT
                % CM -> QM
                \draw[arrow] (CM.south) -- (QM.west) node[label, xshift=0.2cm] {I Quantization}; % Modificato xshift
                % QM -> QFT
...
\begin{table}[H]
    ...
    \begin{tabular}{l c l}
        ...
        \end{minipage}
                                          & \textbf{Measurements}   &
        \begin{minipage}{5cm}
            The result is a spectrum of eigenvalues $\lambda_n$ of $\hat{A}$. The state instantly collapses to the corresponding eigenstate $\ket{\lambda_n}$. The probability of \(\lambda_n\) as an outcome is \(p_n = \vert \langle \psi \vert \lambda_n \rangle \vert^2 = \bra{\psi}\mathbb{P}_n \ket{\psi}\), if \(\ket{\psi}\) is the generic state and \(\ket{\lambda_n}\) is the eigenstate \(\hat{A}\ket{\lambda_n} = \lambda_n \ket{\lambda_n}\).
        \end{minipage} \\
        \midrule
        \begin{minipage}{5cm}
        ...
        \begin{minipage}{5cm}
            ...
            The time evolution operator is unitary and defined by the Hamiltonian.
        \end{minipage}                                                                                                                                                                                                                                                                                                                                                                 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\]
since the outcome of a measure, the eigenvalues of the interesting observable, must be real.
...
\[
    \hat{\mathbb{P}}_{\psi} = \ket{\psi}\bra{\psi} = \begin{pmatrix}
        a_1   \\
        a_2   \\
        \dots \\
        a_n
...
\[
    \hat{\mathbb{P}}^{\dagger} = \hat{\mathbb{P}}, \quad \hat{\mathbb{P}}^2 = \hat{\mathbb{P}}.
\]
...
\[
    \hat{\mathbb{P}}_n\hat{\mathbb{P}}_m = \ket{\psi_n} \langle\psi_n\vert\psi_m\rangle \bra{\psi_n} = \ket{\psi_n}\bra{\psi_n} \delta_{nm},
\]
hence if \(m\neq n\) then the two defining vectors are orthonormal and \(\hat{\mathbb{P}}_n\hat{\mathbb{P}}_m = 0\).
If we sum the projectors, i.d. we apply them consequently, we find intersesting compisitions:
\begin{itemize}
    \item \(\hat{\mathbb{P}}_1 + \hat{\mathbb{P}}_2\) \(2D\): projector on the plane spanned by \(\{\ket{\psi_1},\,\ket{\psi_2}\}\);
    \item \(\hat{\mathbb{P}}_1 + \hat{\mathbb{P}}_2 + \hat{\mathbb{P}}_3\) \(3D\): projector on the volume spanned by \(\{\ket{\psi_1},\,\ket{\psi_2},\,\ket{\psi_3}\}\);
    \item \(\hat{\mathbb{P}}_1 + \dots + \hat{\mathbb{P}}_n\) \(nD\): projector that coincides with the identity:
          \[
              \sum_{n} \hat{\mathbb{P}}_n = \sum_{n} \ket{\psi_n}\bra{\psi_n} = \mathbb{I}_n.
          \]
\end{itemize}
According to the \textit{spectral theorem}, any hermitian operator can be decomposed in:
\[
    \hat{A} = \sum_{n} \lambda_n \hat{\mathbb{P}}_n = \sum_{n} \lambda_n \ket{\psi_n}\bra{\psi_n}.
\]

\paragraph{Measure.}
A measure of an observable \(\hat{A}\) on a state \(\ket{\psi}\) yields a set of possible outcomes \(\lambda_n\) wheighted by a probability \(p_n\) given by
\[
    \ket{\psi}  = \sum_{n} c_n \ket{\psi_n} \quad \implies \quad p_n = \vert c_n \vert^2.
\]
The average value of a measure is given by
\[
    \langle \hat{A} \rangle = \sum_{n} \lambda_n p_n = \bra{\psi} \hat{A} \ket{\psi},
\]
...
\[
    \ket{\psi(t)} = \hat{U}(t)\ket{\psi(0)}, \quad \hat{U}(t) = e^{\tfrac{i}{\hbar} \hat{H} t}.
\]
...
\begin{definition}[Density Matrix of a Pure State]
    A pure density matrix \(\hat{\rho}\) has the following properties:
    \begin{itemize}
        \item it is a bounded operator : \(\Vert \hat{\rho} \Vert \leq 1\);
        \item it is self-adjoint: \(\hat{\rho}^{\dagger} = \hat{\rho}\);
        \item it is a positive operator: \(\bra{\alpha} \hat{\rho} \ket{\alpha} \geq 0 \text{ } \forall \ket{\alpha} \in \mathcal{H}\);
        \item it has unitary trace: \(\Tr(\hat{\rho})=1\);
        \item it is idempotent: \(\hat{\rho}^2 = \hat{\rho}\).
    \end{itemize}
    ...
    \begin{equation}
        \hat{\rho}_{\psi} = \ket{\psi} \bra{\psi}.
        \label{eq:pure_density_matrix}
    \end{equation}
\end{definition}

We can use it to define \textbf{expextation values} for operators, which can be written in terms of the density matrix as
\begin{equation}
    \langle \hat{A} \rangle = \bra{\psi} \hat{A} \ket{\psi} = \Tr(\hat{A} \hat{\rho}_\psi),
    \label{eq:expectation_value_density_matrix}
\end{equation}
where the trace of an operator with elements \(A_{mn} = \bra{\psi_m} \hat{A} \ket{\psi_n}\) is given by
\[
    \Tr(\hat{A}) = \sum_{n} \bra{\psi_n} \hat{A} \ket{\psi_n} = \sum_{n} A_{nn}.
\]
A \textbf{trace-class operator} \(\hat{B}\) is defined as, given any ON basis \(\{\ket{e_j}\}\)
\begin{equation}
    \Tr(\hat{B}) = \sum_{n} \bra{e_n} \hat{B} \ket{e_n} < \infty,
    \label{eq:trace_class_operator}
\end{equation}
...
\[
    \begin{aligned}
        \Tr(\hat{\rho}) & = \sum_{n} \bra{e_n} \hat{\rho} \ket{e_n} = \sum_{n} \bra{e_n} \ket{\psi} \bra{\psi} \ket{e_n}           \\
...
\begin{definition}[Density Matrix of a Mixed State]
    The density matrix of a classical mixture \(\{\ket{\psi_{\alpha}},\,p_{\alpha}\}_{\alpha}\) is given by
    \begin{equation}
        \hat{\rho} = \sum_{\alpha} p_{\alpha} \hat{\rho}_{\alpha} = \sum_{\alpha} p_{\alpha}\ket{\psi_{\alpha}} \bra{\psi_{\alpha}},
        \label{eq:mixed_density_matrix}
    \end{equation}
    where \(\hat{\rho}_{\alpha}\) is the density matrix of the pure state \(\ket{\psi_{\alpha}}\). It has the following properties:
    \begin{itemize}
        \item it is a bounded operator : \(\Vert \hat{\rho} \Vert \leq 1\);
        \item it is self-adjoint: \(\hat{\rho}^{\dagger} = \hat{\rho}\);
        \item it is a positive operator: \(\bra{\alpha} \hat{\rho} \ket{\alpha} \geq 0 \text{ } \forall \ket{\alpha} \in \mathcal{H}\);
...
\begin{example}
    ...
    \[
        \hat{\rho} = \sum_{\alpha} p_{\alpha} \ket{\psi_{\alpha}}\bra{\psi_{\alpha}} = \frac{1}{2} \ket{\uparrow}\bra{\uparrow} + \frac{1}{2} \ket{\downarrow}\bra{\downarrow}
    \]
\end{example}

Notice how the density matrix of a mixed state is \textbf{not idempotent} anymore. Let us analyze further its properties:
\begin{itemize}
    \item Self adjoint operator:
          \[
              \hat{\rho}^{\dagger} = \left( \sum_{\alpha} p_{\alpha} \ket{\psi_{\alpha}}\bra{\psi_{\alpha}} \right)^{\dagger} = \sum_{\alpha} p_{\alpha} \bra{\psi^{\dagger}_{\alpha}} \ket{\psi^{\dagger}_{\alpha}} = \sum_{\alpha} p_{\alpha} \ket{\psi_{\alpha}}\bra{\psi_{\alpha}} = \hat{\rho}.
          \]
    \item Positive operator:
          \[
              \bra{\psi}\hat{\rho}\ket{\psi} = \bra{\psi}\left(\sum_{\alpha} p_{\alpha} \ket{\psi_{\alpha}}\bra{\psi_{\alpha}}\right) \ket{\psi} = \sum_{\alpha} p_{\alpha} \Vert \bra{\psi_{\alpha}}\ket{\psi} \Vert^2 \geq 0.
          \]
    \item Unitary trace:
          \[
              \begin{aligned}
                  \Tr(\hat{\rho}) & = \sum_{n} \bra{e_n}\hat{\rho}\ket{e_n} = \sum_{n} \bra{e_n}\left(\sum_{\alpha} p_{\alpha} \ket{\psi_{\alpha}}\bra{\psi_{\alpha}}\right)\ket{e_n}                                      \\
...
\begin{theorem}
    ...
    \[
        \hat{\rho}^2 = \hat{\rho} \iff \hat{\rho} = \ket{\psi}\bra{\psi}\text{ with } \ket{\psi} \text{ pure.}
    \]
\end{theorem}
\begin{proof}
    \((\impliedby)\) trivial, by definition of density matrix of a pure state.
    \((\implies)\) \(\hat{\rho}\) has to satisfy at least the four properties reported above for a density matrix. Then if we apply the spectral theorem we can write
    \[
        \hat{\rho} = \sum_{n} \lambda_n \ket{e_n}\bra{e_n}, \quad \{\ket{e_n}\} \text{ any ON basis},
    \]
    and this implies
    \[
        \hat{\rho} \ket{e_n} = \lambda_n \ket{e_n},
    \]
    as any hermitian operator acting on an ON basis. Since \(\hat{\rho}\) is positive, we know \(\lambda_n \geq 0\), and since it has unitary trace, we can write
    \[
        \Tr(\hat{\rho}) = 1 \quad \iff \quad 0 \geq \lambda_n \leq 1.
    \]
    If we now compute \(\hat{\rho}^2\), we get
    \[
        \begin{aligned}
            \hat{\rho}^2 & = \left( \sum_{n} \lambda_n \ket{e_n}\bra{e_n} \right) \left( \sum_{m} \lambda_m \ket{e_m}\bra{e_m} \right)                                                                   \\
        ...
        \end{aligned}
    \]
    and by imposing \(\hat{\rho}^2 = \hat{\rho}\) we find out that
    \[
        \hat{\rho}^2 = \hat{\rho} \quad \implies \quad \sum_{n} \lambda_n^2 \ket{e_n}\bra{e_n} = \sum_{n} \lambda_n \ket{e_n}\bra{e_n}
    \]
    ...
    \[
        \hat{\rho} = \sum_{n} \lambda_n \ket{e_n}\bra{e_n} = \lambda_j \ket{e_j}\bra{e_j} = \ket{e_j}\bra{e_j}.
    \]
\end{proof}
...
Lets consider a quantum system in a pure state \(\ket{\psi}\); an observable \(\hat{A}\) (which we remember to be hermitian) will have an expectation value given by
\[
    \langle \hat{A} \rangle_{\psi} = \bra{\psi} \hat{A} \ket{\psi} = \Tr(\hat{A} \hat{\rho}_{\psi}).
\]
Let us show that the last equation holds:
\[
    \begin{aligned}
        \langle \hat{A} \rangle_{\psi} & = \Tr(\hat{A} \hat{\rho}_{\psi}) = \sum_{n} \bra{e_n} \hat{A} \hat{\rho}_{\psi} \ket{e_n}                                        \\
                                       & = \sum_{n} \bra{e_n} \ket{\psi} \bra{\psi} \hat{A} \ket{e_n} = \sum_{n} \bra{\psi} \hat{A} \ket{e_n} \bra{e_n} \ket{\psi}        \\
                                       & = \bra{\psi} \lambda_n \sum_{n} \ket{e_n} \bra{e_n} \ket{\psi} = \bra{\psi} \hat{A} \ket{\psi} = \langle \hat{A} \rangle_{\psi}.
    \end{aligned}
\]
...
\[
    \hat{\rho}_{\psi}  = \sum_{\alpha} p_{\alpha} \hat{\rho}_{\alpha} = \sum_{\alpha} p_{\alpha}\ket{\psi_\alpha}\bra{\psi_\alpha},
\]
then if we compute the expectation value of \(\hat{A}\):
\[
    \begin{aligned}
        \langle \hat{A} \rangle_{\psi} & = \Tr(\hat{A} \hat{\rho}_{\psi}) = \Tr\left(\sum_{\alpha} p_{\alpha} \hat{\rho}_{\alpha} \hat{A} \right) = \sum_{\alpha} p_{\alpha} \Tr(\hat{\rho}_{\alpha} \hat{A}) = \sum_{\alpha} p_{\alpha} \langle \hat{A} \rangle_{\alpha},
    \end{aligned}
\]
which is the sum of the expectations value of \(\hat{A}\) in each of the pure states, weighted by the probability of the mixed state to be observed in that state.\footnote{A clearer but heavier notation would have called \(\hat{\rho}_{\alpha} \to \hat{\rho}_{\psi_{\alpha}}\) and \(\langle \hat{A} \rangle_{\alpha} \to \langle \hat{A} \rangle_{\psi_\alpha}\).}
...
To clarify further, let's take two particles \(e\) and \(f\), both of which can be observed in the state \(\ket{\cdot}_1\) or \(\ket{\cdot}_2\): we have two possible global states
\[
...
We can study the simplest transformation of this group, the \textbf{elementary permutation} (or transposition) exchanging the \(i-\)th element with the \((i+1)-\)th one, in order to understand better this group and his properties. The elementary permutation is so important because it is the \textbf{generator} of this transformation group: we can write any other permutation by composing different elementary permutations. Thus \(\sigma_i\) with \(i = 1,\,\dots,\,N\) are the generators of the group, but decomposing a permutation in such way is not uniquely defined:
\[
...
If we consider a generic state of \(N\) indistinguishable particles \(\psi(1,2,\dots ,N) \in \mathcal{H}^{\otimes N}\), invariant under permutation up to a phase (which we will not assume permutation independent to be general):
\[
...
We can introduce two operators, \(\hat{S}\) and \(\hat{A}\) which project our state onto the totally antisymmetric or symmetric subspaces as follows:
\[
    \begin{aligned}
        \hat{S} \psi & = \psi_S (x_1,\,x_2) = \frac{\psi(x_1,\,x_2) + \psi(x_2,\,x_1)}{2} = \frac{\psi_1(x_1)\psi_2(x_2) + \psi_1(x_2)\psi_2(x_1)}{2} = \psi_S(x_2,\,\psi_1);   \\
        \hat{A} \psi & = \psi_A (x_1,\,x_2) = \frac{\psi(x_1,\,x_2) - \psi(x_2,\,x_1)}{2} = \frac{\psi_1(x_1)\psi_2(x_2) - \psi_1(x_2)\psi_2(x_1)}{2} = - \psi_A(x_2,\,\psi_1).
    \end{aligned}
\]
...
\[
    \begin{aligned}
        \hat{S} & = \hat{S}^{\dagger},\quad \hat{S}^2 = \hat{S}; \\
        \hat{A} & = \hat{A}^{\dagger},\quad \hat{A}^2 = \hat{A}.
    \end{aligned}
\]
...
\[
    \hat{S}\hat{A} = \hat{A}\hat{S} = 0,\quad \bra{\psi_S}\ket{\psi_A} = \bra{\psi_A}\ket{\psi_S} = 0.
\]
...
\]
we know this state to be antisymmetric, so if we apply the \(\hat{A}\) operators we are not changing the state nor projecting it into a subspace, it's the same; then by applying it we notice how the wavefunction vanishes:
\[
    \hat{A} \psi(x_1,\,x_2) = \frac{\psi_1(x_1)\psi_1(x_2) - \psi_1(x_2)\psi_1(x_1)}{2} = 0.
\]
...
\[
    \begin{aligned}
        \hat{S} \colon \ket{\psi} & = \frac{1}{N!} \sum_{\sigma} \ket{\psi_{\sigma(1)}}_1\ket{\psi_{\sigma(2)}}_2\dots\ket{\psi_{\sigma(N)}}_N \in \mathcal{H}_S;                      \\
        \hat{A} \colon \ket{\psi} & = \frac{1}{N!} \sum_{\sigma} \mathrm{sgn}(\sigma) \ket{\psi_{\sigma(1)}}_1\ket{\psi_{\sigma(2)}}_2\dots\ket{\psi_{\sigma(N)}}_N \in \mathcal{H}_A.
    \end{aligned}
\]
