...
\begin{itemize}
    \item For \(T > T_{BEC}\), there is no condensation, so all particles are in excited states:
          \[
              n_0(T) = 0, \quad n_{n}(T) = \frac{g}{\lambda_{T}^3} g_{3/2}(z).
          \]
    \item For \(T < T_{BEC}\), condensation occurs, so we have a macroscopic occupation of the ground state:
          \[
              n_0(T) = n \left[ 1 - \left( \frac{T}{T_{BEC}} \right)^{3/2} \right], \quad n_{n}(T) = \frac{g}{\lambda_{T}^3} \zeta(3/2) = n \left( \frac{T}{T_{BEC}} \right)^{3/2}.
          \]
\end{itemize}
...
We can now compute the energy per particle \(E/N\) in the two temperature regimes:
\begin{itemize}
    \item For \(T > T_{BEC}\):
          \[
    ...
    \item For \(T < T_{BEC}\):
          ...
          \]
\end{itemize}
Note that we have used the expression for the normal component \(n_{n}(T)\) to relate the energy to the total number of particles \(N\): for \(T < T_{BEC}\), only the normal component contributes to the energy
\[
    N_n(T) = N \left( \frac{T}{T_{BEC}} \right)^{3/2} = \frac{gV}{\lambda_{T}^3} \zeta(3/2),
\]
and we are computing the expression for \(u=\frac{E}{N}\frac{V}{V}\) accordingly.
