...
\]
with diagonal components \(F^{00} = F^{ii} = 0\).
...
\]
This form is convenient both for solving classical radiation problems and for quantization, since each component \(A^\nu\) satisfies a standard sourced wave equation. Gauge fixing removes the redundancy associated with gauge transformations and therefore eliminates non-physical degrees of freedom; after imposing the Lorenz condition and using the residual gauge freedom one can remove further unphysical components and isolate the two transverse physical polarizations of the photon. \QUESTION{Did I really remove the degree of freedom?}
...
\[
    F^{\mu \lambda} F_{\lambda}^{\ \nu} = F^{\mu}_{\ \alpha} F^{\beta \nu} \eta^{\lambda \alpha} \eta_{\lambda \beta} = F^{\mu}_{\ \lambda} F^{\lambda \nu} = F^{\lambda \nu} F^{\mu}_{\ \lambda} = \underbrace{F^{\nu \lambda}}_{-F^{\lambda \nu}} \underbrace{F_{\lambda}^{\ \mu}}_{-F^{\mu}_{\ \lambda}}.
\]
...
\[
    \begin{aligned}
        \mathcal{P}^i & = \tilde{T}^{0i} = F^{0 j} F_{j}^{\ i} +\frac{1}{4} \eta^{0i}\left(---\right)          \\
                      & = F^{0 j} F_{j}^{\ i} = (-E^i)(\epsilon^{ijk} B^k) = (\mathbf{E} \times \mathbf{B})^i,
    \end{aligned}
\]
