...
\[
    \partial_i \hat{A}_{\mu}(\mathbf{x}) = \int \frac{\mathrm{d}^3 \mathbf{p}}{(2\pi)^3} \frac{-i \mathbf{p}}{\sqrt{2 \vert \mathbf{p} \vert}} \sum_{\lambda=0}^{3} \epsilon_{\mu}^{(\lambda)}(\mathbf{p}) \left( \hat{a}^{(\lambda)}_{\mathbf{p}} e^{i p^j x^j} - \hat{a}^{(\lambda)\,\dagger}_{\mathbf{p}} e^{-i p^j x^j} \right),
\]
since \(\partial_i e^{i p^j x^j} = \frac{\partial}{\partial x^i} e^{i p^j x^j} = i p^i e^{i p^j x^j} = -i p_i e^{i p^j x^j}\).\footnote{When computing the contraction \(\partial_i \hat{A}_{\mu}\partial^i \hat{A}^{\mu}\), we have to pay attention to the other derivative, since \(\partial^i e^{i p^j x^j} = \frac{\partial}{\partial x_i} e^{i p^j x^j} = i p^i \eta_{ij} e^{i p^j x^j} = i p_i e^{i p^j x^j}\); thus in the product we have a minus sign.} Thus, after integrating in \(\mathrm{d}^3 \mathbf{x}\) and \(\mathrm{d}^3 \mathbf{q}\) in order to resolve the delta functions, we find
\[
    \begin{aligned}
...
\]
Summing these two contributions, we find that the terms with two creation or two annihilation operators cancel out since the opposite sign in the momentum leads to ... (?).\QUESTION{Why do they cancel out?} Thus, after normal ordering the expression (we will not us the explicit notation), we find
\[
    \begin{aligned}
