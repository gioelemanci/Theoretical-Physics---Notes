\subsection{Finite-dimensional representations} \label{sec:finite_dimensional_representations}
...
\begin{equation}
    ...
    \label{eq:vectorial_representation_generators}
\end{equation}
...
\]
This representation acts on four-vectors and mixes their components under rotations and boosts.
...
\begin{equation}
    \sigma^{\mu} = (\mathbb{I}, \boldsymbol{\sigma}), \quad \overline{\sigma}^{\mu} = (\mathbb{I}, -\boldsymbol{\sigma}),
    \label{eq:sigma_matrices}
\end{equation}
where \(\boldsymbol{\sigma} = (\sigma^1, \sigma^2, \sigma^3)\) are the \textbf{Pauli matrices} in \eqref{eq:Pauli_matrices}.
...
\begin{enumerate}
    \item \textbf{Left-handed Weyl spinors} (fundamental representation \((\tfrac{1}{2},0)\)):
          ...
          \begin{equation}
              ...
              \label{eq:LH_Weyl_spinor_transformation}
          \end{equation}
    ...
    \item \textbf{Right-handed Weyl spinors} (complex conjugate representation \((0,\tfrac{1}{2})\)):
          ...
          \begin{equation}
              \bar{\chi}^{\dot{\alpha}} =
              \begin{pmatrix}
                  \bar{\chi}^{\dot{1}} \\ \bar{\chi}^{\dot{2}}
              \end{pmatrix}
              \longrightarrow
              \bar{\chi}'^{\dot{\alpha}} = N^{* \dot{\alpha}}_{\ \ \dot{\beta}} \bar{\chi}^{\dot{\beta}}, \qquad \dot{\alpha},\dot{\beta} = 1,2.
              \label{eq:RH_Weyl_spinor_transformation}
          \end{equation}
          These spinors transform as right-handed under spatial rotations and so thay form the representation \(\mathrm{SU}(2)_R\).
\end{enumerate}
...
The infinitesimal generators of \(\mathrm{SL}(2,\mathbb{C})\) correspond to those of the Lorentz algebra and are represented as\footnote{Where \(\overline{\sigma}^{\mu} = \left(\mathbb{I}, -\bs{\sigma} \right)\), as defined in eq. \eqref{eq:sigma_matrices}.}
\[
    \left\{
    \begin{aligned}
...
\begin{equation}
    ...
    \begin{pmatrix}
        ...
        0                & \bar{\sigma}^{\mu \nu}
    \end{pmatrix},
    \label{eq:Dirac_representation_generators}
\end{equation}
...
The Dirac conjugate spinor is defined as\footnote{The \textit{bar} and the \textit{dotted index} are distinct concepts exploiting the same property of the Weyl spinor: the bar denotes the conjugate Weyl representation (\(\psi \in \mathrm{SU}(2)_L\), while \(\overline{\psi} \in \mathrm{SU}(2)_R\)), while the dotted index indicates transformation under the conjugate representation: when we are operating the complex conjugate of a left-handed spinor, we need to introduce dotted index and bar, while for the conjugation of a RH spinor we need the opposite.}
\begin{equation}
    \bar{\Psi} = \Psi^{\dagger} \gamma^0 =
    \begin{pmatrix}
        \overline{\chi}^{*\dot{\alpha}} & \psi^*_{\alpha}
    \end{pmatrix} = \begin{pmatrix}
        \chi^{\alpha} & \overline{\psi}_{\dot{\alpha}}
    \end{pmatrix}.
    \label{eq:Dirac_conjugate}
\end{equation}
