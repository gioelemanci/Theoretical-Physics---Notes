...
\begin{enumerate}[label=(\roman*),align=left,itemsep=1.0\baselineskip]
    \item \textbf{Integral on the right arc at infinity vanishes}, \(\lim_{\vert p \vert \to \infty} \int_{C_1} f(p)\,dp = 0\):\\
          ...
          \]
          as \(\vert p \vert \to \infty\) while mantaining \(r>t\): after regrouping the real parts (which are multiplied by a \(i\) making the exponantial oscillate) in \(\overline{\theta}\), we can se how the second exponential dominate the third when \(r \gg t\) (space-like separated events).
