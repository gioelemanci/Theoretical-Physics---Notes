...
\[
    i\hbar \frac{\partial}{\partial t} \phi(\mathbf{x},t)
    = \sqrt{-\hbar^2 c^2 \nabla^2 + m^2 c^4} \, \phi(\mathbf{x},t).
\]
...
\begin{equation}
    \phi^{(+)}_{\mathbf{p}}(x) = e^{-i E_p t + i \mathbf{p} \cdot \mathbf{x}},
    \label{eq:KG_positive_energy_plane_wave}
\end{equation}
while the negative-energy modes are given by
\begin{equation}
    \phi^{(-)}_{\mathbf{p}}(x) = e^{i E_p t - i \mathbf{p} \cdot \mathbf{x}}.
    \label{eq:KG_negative_energy_plane_wave}
\end{equation}
...
\begin{equation}
    ...
    \left[
        ...
        \right],
    \label{eq:KG_general_solution_fourier}
\end{equation}
...
\[
    ...
    \end{aligned}
\]
which is zero because the starting expression vanishes identically (literally the KG equation \eqref{eq:Klein_Gordon_equation}, which is zero, multiplied by something).
...
We thus conclude that the Klein–Gordon equation cannot sustain a probabilistic interpretation in terms of a positive-definite probability density: the quantity $J^0$ cannot be interpreted as a probability density \(\tfrac{\rho}{c}\).
...
\begin{figure}[H]
    \begin{minipage}{0.5\textwidth}
        ...
        \[
            \frac{ke^{ikr}}{k^2 + m^2} = \frac{ke^{ikr}}{(k+im)(k-im)},
        \]
...
\[
    \begin{aligned}
        \phi(\mathbf{x}) & = -\frac{g}{(2\pi)^2 ir} (2\pi i)\sum_{i} \mathrm{Res} f(k) \Big|_{k^+}  \\
                         ...
                         & = -\frac{g}{2\pi r} \frac{im e^{-mr}}{2im} = -\frac{g}{4\pi r} e^{-mr}.
    \end{aligned}
\]

This potential describes an attractive interaction between sources of the same sign. Its range is finite and characterized by \(\lambda \sim \tfrac{1}{m}\), corresponding to the \textbf{Compton wavelength} of a particle of mass $m$. Hence, the Yukawa potential models \textit{short-range forces}, such as the nuclear force.
One can verify that \(\phi(\mathbf{x}) = -\tfrac{g}{4\pi r} e^{-mr}\) satisfies equation \eqref{eq:Klein_Gordon_static_source} by direct substitution.
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
As we have said, the potential has a characteristic range scaling inversely with the mass of the exchanged particle: the behaviour of a short-range force
\[
    R \sim \frac{1}{m},
\]
which is soppresressed exponentially for distances \( r \gg R \).
...
\[
    m \sim 197~\text{MeV},
\]
in remarkable agreement (the same order of magnitude) with the mass of the neutral pion, $m_{\pi^0} \approx 135~\text{MeV}$, discovered later in cosmic-ray experiments.
...
\[
    \begin{aligned}
        ...
        = \int \mathrm{d}^4 y \, (-\Box + m^2) G(x - y) J(y) = \int \mathrm{d}^4 y \, \delta^{(4)}(x - y) J(y) = J(x).
    \end{aligned}
\]
The Green function thus acts as a \textit{kernel that ``propagates'' the influence of the source} \(J(y)\) from each spacetime point \(y\) to the observation point \(x\).
...
This prescription enforces that positive-frequency components (associated with particles of positive energy) propagate forward in time, while negative-frequency components (associated with antiparticles with negative energy) propagate backward in time (changing the sign of their energy).
...
The corresponding \textit{Feynman propagator} can be expressed in momentum space as a fourier transform:
\begin{equation}
...
\]
The infinitesimal term \(-i\epsilon\) (with \(\epsilon \to 0^+\)) determines the position of the poles in the complex \(p^0\)-plane and thus implements the causal boundary conditions (Feynman-Stueckelberg) described above.
...
In this sense, negative-energy states are reinterpreted as antiparticles with positive energy propagating backward in time.
...
\[
    \begin{aligned}
        ...
        \frac{-i }{p^2 + m^2}e^{ip_\mu (x^{\mu} - y^{\mu})}                                                                                                           \\
...
\begin{remark}
    ...
    In quantum field theory, however, \(\phi(x)\) becomes an operator, and the relevant physical quantity is the \textit{Feynman propagator} \(\Delta(x - y)\) which measures the amplitude for a quantum of the field created at \(y\) to be annihilated at \(x\).
...
\begin{figure}[H]
    \begin{minipage}{0.4\textwidth}
        ...
        \begin{tikzpicture}[scale=0.8, every node/.style={font=\small}]
            ...
            \fill[green!60!black] (-\R/2,  \eps) circle (1.5pt) node[above] {\small $-E_p + i\epsilon$};
            \fill[green!60!black] (\R/2,  -\eps) circle (1.5pt) node[below] {\small $E_p - i\epsilon$};
            \fill[green!60!black] (-\R/2, 0) circle (1.5pt) node[below] {\small $-E_p$};
            \fill[green!60!black] (\R/2, 0) circle (1.5pt) node[above] {\small $E_p$};
...
This requires distinguishing two cases according to the sign of the exponential, and hence choosing whether to close the contour with \(\mathcal{C}_{\text{upper}}\) or \(\mathcal{C}_{\text{lower}}\), depending on the sign of \(x^0 - y^0\).
...
\]
with $y^\mu$ the spacetime point that supports the external source.

The \textbf{retarded Green function} $G_R(x - y)$ is defined to propagate all frequencies excited by the source at the spacetime point $y^\mu$ forward in time, so that $G_R(x - y)$ vanishes for $x^0 < y^0$. It is fixed by displacing all the poles below the real $p^0$ axis.

The \textbf{advanced Green function} $G_A(x - y)$ is defined to propagate all frequencies backward in time so that it vanishes for $x^0 > y^0$. It is obtained by displacing the poles above the real $p^0$ axis.
...
\subsection{Action and Lagrangian formalism}
...
A brief review of the action formalism and Noether’s theorem is given in Appendix \ref{app:Noether}.\TODO{Finish appendix}
...
\[
    ...
    \end{aligned}
\]
where we have integrated by parts and discarded surface terms assuming that the variations $\delta \phi$ and $\delta \phi^{\ast}$ vanish at infinity.

Since the variations $\delta \phi$ and $\delta \phi^{\ast}$ are arbitrary, the integrand must vanish independently for each, we obtain to the Euler–Lagrange equations:
\[
    \begin{dcases}
        (\Box - m^2)\phi(x)         = 0, \\
        (\Box - m^2)\phi(x)^{\ast}  = 0.
...
\[
    ...
    \frac{\delta \mathcal{L}[\phi, \phi^{\ast}]}{\delta \phi(x)} \, \delta \phi(x)
    + \frac{\delta \mathcal{L}[\phi, \phi^{\ast}]}{\delta \phi^{\ast}(x)} \, \delta \phi^{\ast}(x)
    \right) = 0,
\]
and imposing suitable boundary conditions (so that surface terms vanish), one obtains the field equations
\[
    \frac{\delta \mathcal{L}[\phi, \phi^{\ast}]}{\delta \phi^{\ast}(x)} = (\Box - m^2)\phi(x) = 0,
    \qquad
    \frac{\delta \mathcal{L}[\phi, \phi^{\ast}]}{\delta \phi(x)} = (\Box - m^2)\phi^{\ast}(x) = 0.
\]
...
\begin{enumerate}
    \item \textbf{Space–time symmetries}, described by the \textit{Poincaré group}.
    \item \textbf{Internal symmetries}, associated with the invariance under phase rotations belonging to the group \(\mathrm{U}(1)\).
\end{enumerate}
The term “rigid” (or equivalently “global”) indicates that these transformations do not depend on the spacetime point \(x\): the same transformation is applied everywhere, unlike the case of \textit{gauge symmetries}, where the transformation parameters are local functions of \(x\).
...
It is straightforward to verify that under this transformation the action in \eqref{eq:KG_action_lagrangian} remains invariant:\footnote{Since we can always factorize the two exponentials out of the derivatives (\(\phi\) and \(\phi^{\ast}\) appear multiplied together in the Lagrangian), the invariance holds for any real value of \(\alpha\).}
\[
...
In this case, the action is no longer invariant, and taking an infinitesimal transformation, the variation becomes
\begin{equation}
...
\begin{equation}
    Q \equiv \int \mathrm{d}^3 \mathbf{x} \, J^0
    = -i \int \mathrm{d}^3 \mathbf{x} \, \phi^{\ast}
...
\[
    ...
    \equiv \int \mathrm{d}^3 \mathbf{x} \, i \chi^{\ast}
    \!\stackrel{\leftrightarrow}{\partial_0}\!\phi.
\]
...
\[
    \begin{aligned}
        \delta_a \phi(x) & = \phi^{\prime} (x) - \phi(x) = \phi^{\prime} (x) - \phi^{\prime} (x^{\prime})                                                        \\
                         ...
                         & = - \left(\phi^{\prime} (x) + a^{\mu}\partial_\mu \phi^{\prime}(x) - \phi^{\prime} (x)\right) = -a^{\mu}\partial_\mu \phi^{\prime}(x)
    \end{aligned}
\]
where we have Taylor expanded, so that eq. \eqref{eq:Poincaré_symmetries} becomes\footnote{Since the field is multiplied by an infinitesimal displacement \(a^{\mu}\), we can replace \(\phi^{\prime}\) with \(\phi\) in the last step without affecting the result to first order in \(a^{\mu}\).}
\begin{equation}
    \begin{aligned}
...
\]
Here, total derivatives have been neglected, and \(\mathcal{L}\) denotes the Lagrangian density from eq.~\eqref{eq:KG_action_lagrangian}: this result can be obtained considering linearity of the lagrangian, thus we can compute the variation of each term separately and directly in the lagrangian
\[
    \begin{aligned}
        \delta_{a(x)} \mathcal{L}[\phi, \phi^{\ast}]
         & = -(\partial_{\mu} \delta_{a(x)} \phi^{\ast}) \, (\partial^{\mu} \phi) - (\partial_{\mu} \phi^{\ast}) \, (\partial^{\mu} \delta_{a(x)} \phi) - m^2 \left(\delta_{a(x)} \phi^{\ast} \, \phi + \phi^{\ast} \, \delta_{a(x)} \phi\right)                                                                    \\
         & = \partial_{\mu} (a^{\nu} \partial_{\nu} \phi^{\ast}) \, (\partial^{\mu} \phi) + (\partial_{\mu} \phi^{\ast}) \, \partial^{\mu} (a^{\nu} \partial_{\nu} \phi) + m^2 \left(a^{\nu} \partial_{\nu} \phi^{\ast} \, \phi + \phi^{\ast} \, a^{\nu} \partial_{\nu} \phi\right)                                 \\
         & = \left[ (\partial_\mu a_{\nu})\partial^{\nu} \phi^{\ast} + a^{\nu} (\partial_\mu \partial^{\nu} \phi^*) \right] \partial^{\mu} \phi + \partial^{\mu} \phi^{\ast} \left[ (\partial_{\mu} a_{\nu}) \partial^{\nu} \phi + a_{\nu} (\partial_{\mu} \partial^{\nu} \phi) \right] + m^2 (\cdot)               \\
         & = (\partial_{\mu} a_{\nu}) \left( \partial^{\mu} \phi^{\ast} \partial^{\nu} \phi + \partial^{\nu} \phi^{\ast} \partial^{\mu} \phi \right) + a_{\nu} \left[(\partial_\mu \partial^{\nu} \phi^*)\partial^{\mu} \phi + \partial^{\mu} \phi^{\ast} (\partial_{\mu} \partial^{\nu} \phi)\right] + m^2 (\cdot) \\
         & = (\partial_{\mu} a_{\nu}) \left( \partial^{\mu} \phi^{\ast} \partial^{\nu} \phi + \partial^{\nu} \phi^{\ast} \partial^{\mu} \phi \right) - \eta^{\mu\nu} \partial_\mu a_\nu \left( \partial_\mu \phi^* \partial^{\mu} \phi \right) + m^2 (\cdot),
    \end{aligned}
\]
where in the last step we have integrated by parts the second term, yielding a total derivative that can be neglected (in the end we are interested only in the action, and we assume boundary terms vanish)
\[
    a^{\nu} (\partial_\mu \partial^{\nu} \phi^*)\partial^{\mu} \phi = \partial_\nu (\cdot) - \eta^{\nu\lambda} \partial_\lambda a_\nu (\partial_\mu \phi^* \partial^{\mu} \phi) - a_\nu \partial^{\mu} \phi^{\ast} (\partial_{\mu} \partial^{\nu} \phi),
\]
from which the final expression follows. Now, rearranging the first mass term with the idea to construct the lagrangian density (multiplied by the Minkowski metric), and a deletion for the remaining mass term, we obtain
\[
    m^2 \left(a^{\nu} \partial_{\nu} \phi^{\ast} \, \phi\right) = m^2 \partial_\nu (\cdot) - m^2 \eta^{\nu \lambda} \partial_\lambda a_\nu (\phi^* \phi) - m^2 \phi^* a^\nu \partial_\nu \phi,
\]
where again the total derivative can be neglected, leading to the final form of the variation:
\[
    \delta_{a(x)} \mathcal{L}[\phi, \phi^{\ast}] = (\partial_{\mu} a_{\nu}) \left( \partial^{\mu}\phi^{\ast}\partial^{\nu}\phi + \partial^{\nu}\phi^{\ast}\partial^{\mu}\phi + \eta^{\mu\nu} \mathcal{L} \right),
\]
from which the variation of the action follows immediately and thus the identification of the energy-momentum tensor:
\begin{equation}
...
The corresponding conserved four-vector, the Noether charge associated with translations, is given by
\[
    P^{\mu} = \int \mathrm{d}^3 \mathbf{x} \, T^{0\mu},
\]
represents the total \textbf{four-momentum} of the field.
Each component of \(P^{\mu}\) is linked to a specific symmetry:
\begin{itemize}
    \item The temporal component \(P^{0}\) is conserved in \textbf{time translations} and represents the total \textit{energy} of the field.
    \item The spatial components \(P^{i}\) (\(i = 1, 2, 3\)) are conserved in \textbf{spatial translations} and represent the components of the \textit{momentum vector}.
\end{itemize}

Explicitly, the \textbf{local energy density} is given by
\[
    \begin{aligned}
        T^{00} & = 2\partial^0 \phi^{\ast} \partial^0 \phi - \eta^{00} \left(\partial_{\lambda} \phi^{\ast} \partial^{\lambda} \phi - m^2 \phi^{\ast} \phi\right) \\
               & = 2\partial_0 \phi^{\ast}\partial_0 \phi +\partial_0 \phi^* \partial^0 \phi + (\nabla\phi^{\ast}) \cdot (\nabla\phi) + m^2 \phi^{\ast}\phi,
    \end{aligned}
\]
so that we can write
\begin{equation}
    \mathcal{E}(x) = \partial_0 \phi^{\ast} \partial_0 \phi + (\nabla\phi^{\ast}) \cdot (\nabla\phi) + m^2 \phi^{\ast}\phi.
    \label{eq:energy_density_KG}
\end{equation}
The total energy of the field is therefore
\[
    P^{0} \equiv E = \int \mathrm{d}^3 \mathbf{x} \, T^{00}.
\]
