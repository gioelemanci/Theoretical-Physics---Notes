...
\begin{example}[Green's function for free electrons]
    ...
    \[
        \hat{H} = \sum_{\mathbf{k}, \sigma} \xi_{\mathbf{k}} \hat{c}_{\mathbf{k}, \sigma}^\dagger \hat{c}_{\mathbf{k}, \sigma},
    \]
    where \(\xi_{\mathbf{k}}\) is the energy of an electron with momentum \(\mathbf{k}\). The Green's function for this system can be computed using the definition:
    \[
        \mathcal{G}^R(\mathbf{k}, \sigma, t; t') = -i \theta(t - t') \langle \{ \hat{c}_{\mathbf{k}, \sigma}(t),\, \hat{c}_{\mathbf{k}, \sigma}^\dagger(t') \} \rangle.
    \]
