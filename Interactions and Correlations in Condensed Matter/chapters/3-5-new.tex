...
since we are considering integrals of a sort of hyperbolic tangent, which it is approximately equal to \(1\) for \(\epsilon \gg k_B T\), and it is approximately equal to \(-1\) for \(\epsilon \ll k_B T\) (furthermore linear in \(\epsilon\) for \(\epsilon \sim k_B T\)) we can understand that
\begin{itemize}
    \item the first integral gives a contribution of the order of \(D(E_F)(k_B T)\), since in the regime of the first integral the hyperbolic tangent is approximately linear with \(\epsilon\);
    \item the second integral gives a contribution of the order of \(D(E_F) \ln(\frac{\omega_D}{k_B T})\), which is positive and finite, since the integral is dominated by the ending part of the hyperbolic tangent, which is approximately equal to \(1\) for \(\epsilon \gg k_B T\).
\end{itemize}
