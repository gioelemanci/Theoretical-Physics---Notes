...
It is worth noting that a node might have a very low degree $k$ but a disproportionately high betweenness centrality. Such nodes act as \textit{brokers} or \textit{bridges} between dense clusters. In a typical scatterplot of $BC$ versus $k$, these nodes are easily identifiable as outliers that deviate from the expected positive correlation.

\paragraph{Limitations.} The fundamental hypothesis of BC is that interactions occur \textit{only} along the shortest paths. In many real-world scenarios, this is an oversimplification: longer paths might be preferred to avoid traffic jams or bottlenecks, requiring the introduction of further topological weights.

\subsection{Normalized Betweenness}

The absolute value of Betweenness Centrality scales with the network size $N$, because larger networks naturally possess more paths. To compare the betweenness of nodes belonging to different networks, a proper normalization is required.
...
When a network exhibits very high normalized betweenness values, it structurally resembles a tree: a very sparse graph with a distinct absence of cycles that could generate alternative paths.

\subsection{Betweenness and Global Efficiency}

Betweenness centrality is intimately connected to the concept of network efficiency. The \textbf{Global Efficiency} ($E$) of a network measures its overall capability to transfer information and is defined as the mean of the inverse of the lengths of all shortest paths (often referred to as a "harmonic composition"):
