...
Given N and P I do not have a fixed number of
networks belonging to the canonical ensemble
Better: ALL graphs with N nodes belong to GN,P with
a different probability depending on P
The probability of having a graph with m links and N
nodes is governed by the binomial distribution:
\[
    P(m) = \binom{N(N-1)/2}{m} P^m (1-P)^{N(N-1)/2-m}
\]

The average grows as the ...
...
\chapter{Spectral Analysis of Random Graphs}

We are going to analyze the spectral properties of random graphs, which are important for understanding the dynamics and functionality of complex networks. We will start with the spectral density of the adjacency matrix, which is a key tool for understanding the structure of random graphs. We will then explore the eigenvalue distribution and its implications for the stability and dynamics of random graphs.

The \textbf{observables} of interest in the study of random graphs include
\begin{itemize}
    \item Eigenvalue distribution of the adjacency matrix, which can provide insights into the structure and dynamics of the network.
    \item Maximum eigenvalue, which can be related to the stability of the network and the presence of a giant component.
    \item Nearest neighbor spacing distribution (NNSD), which can provide insights into the correlations between eigenvalues and the presence of community structure in the network.
\end{itemize}
...
eigenvalue spctrum er network


For a random symmetric NxN matrix with elements sampled from a Gaussian pdf \(N(0, \sigma)\) the eigenvalue spectrum has the following form. Real spectrum: semicircle centered around origin with a radius depending on node number N and Gaussian SD \(\sigma\):
\[
    \rho(\lambda) = \frac{1}{2\pi N \sigma^2} \sqrt{4N\sigma^2 - \lambda^2}, \quad |\lambda| < 2\sigma \sqrt{N}.   \dots
\]

ALL normalized eigenvector components are distributed as a Gaussian pdf: ALL eigenvectors are EQUAL and “poorly informative”

There will be no subsets of nodes particularly emerging from components of our eigenvectors, as they are all equally distributed. This is a key feature of random graphs, where there is no specific structure or correlation in the network, and the eigenvectors do not provide any meaningful information about the structure of the graph.
...
\subsubsection{E-R Spectrum}

If we take an E-R networks with 0-1 links maximally distributed in a random way, wigner semicircle law will still hold, but with a different radius depending on the link density. It is not exactely the gaussian orthogonal enamble, but it is still a random symmetric matrix with entries that are randomly distributed. The eigenvalue spectrum of an E-R random graph will still follow the Wigner semicircle law, but the radius of the semicircle will depend on the link density of the graph, and we have a big eigenvalue \(\lambda_{\max}\) that emerges from the bulk of the spectrum, which is related to the presence of a giant component in the graph. The presence of this large eigenvalue is a key feature of E-R random graphs and distinguishes them from more structured networks, where there may be multiple large eigenvalues corresponding to different communities or other structures in the graph.

The \textbf{maximum eigenvalue} of the adjacency matrix of a network is bound by the average degree \(\langle k \rangle\) and the largest degree \(k_{max}\) of the corresponding network as follows:
\[
    \langle k \rangle \leq \lambda_{\max} \leq k_{max}.
\]
The largest eigenvalue has emerged as a key quantity for the study of a variety of systems and processes, such as virus spread, synchronization of coupled oscillators and stability of couplings in the brain.
