...
\TODO{Insert image showing the evolution of dumbbell graphs for n=3 to n=8.}
\TODO{Insert image showing a binary tree graph.}
\TODO{Insert image showing a 3D grid graph.}
...
\TODO{Insert image showing a bipartite dumbbell graph with N=10 and KMAX=5, where nodes are visually separated into two distinct clusters connected by a single central link, with lambda2 = 0.29.}
...
\begin{example}[Vibrational Analogy and Nodal Points]
    ...
    The points where the eigenvector components cross zero are called \textbf{nodal points}.
    \TODO{Insert image of vibrating ropes showing the standing waves and the nodal points intersecting the central horizontal axis.}
\end{example}
...
\begin{example}
    ...
    \TODO{Insert image of a simple path graph with 5 nodes connected in a line.}
...
\]
And then convert it into an adjacency matrix using a Gaussian kernel:
\[
    A_{ij} = e^{- \frac{s_{ij}^2}{\sigma^2}}
\]
\TODO{Insert image showing the S\&P 500 correlation function plot and the resulting heatmaps comparing 1-spectral, 2-spectral, and 3-spectral centralities against Degree, Betweenness, and Eigenvector centralities across different equity windows.}
...
\begin{example}[Spectral Centrality vs. Efficiency (Biological Example)]
    ...
    \TODO{Insert scatter plot comparing Spectral Centrality SC1 (log scale) vs. Efficiency E for biological immune system markers like CSF-1, IL-7, TGF-beta, etc.}
