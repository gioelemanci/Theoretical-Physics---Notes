...
\[
    Z = \int \mathcal{D}m(\mathbf{x}) e^{-\int \mathrm{d}^d \mathbf{x} \left[ \frac{t}{2}m^2(\mathbf{x}) + u m^4(\mathbf{x}) + \frac{\kappa}{3}(\nabla m)^2 \right]}.
\]
...
Since we have defined \(m(\mathbf{x}) = \overline{m} + \phi(\mathbf{x})\), we can perform explicitly the functional integral over \( \phi(\mathbf{x}) \)
\[
...
\[
    Z = \left( \prod_{\mathbf{q}} \int \mathrm{d}\phi_{\mathbf{q}} \right) e^{- \int \frac{\mathrm{d}^d \mathbf{q}}{(2\pi)^d} (c_2 q^2 + c_1) |\phi_{\mathbf{q}}|^2},
\]
...
\[
    Z = \prod_{\mathbf{q}}' \left[ \int \mathrm{d}\phi_{\mathbf{q}} \, e^{- \frac{1}{V} (c_2 q^2 + c_1) |\phi_{\mathbf{q}}|^2} \right]
\]
...
\[
    Z = \sqrt{\prod_{\mathbf{q}} \left[ \int \mathrm{d}\phi_{\mathbf{q}} \, e^{- \frac{1}{V} (c_2 q^2 + c_1) |\phi_{\mathbf{q}}|^2} \right]},
\]
...
\[
    C \simeq T a^2 \frac{\partial^2}{\partial t^2} \left[ \frac{1}{\beta} \log Z \right] \simeq T_c a^2 \frac{\partial^2}{\partial t^2} \left[ T_c \log Z(t) \right].
\]
...
\begin{itemize}
    \item First, let us recall the \textbf{saddle-point approximation} for \( Z_{LG} \) at \( h=0 \), given by
          \[
          ...
          \[
              \log Z_{LG} \simeq -V \left( \frac{t}{2}\overline{m}^2 + u \overline{m}^4 \right) = \begin{dcases}
                  0                  & \text{for } t > 0, \\
    ...
    \item Next, let us compute how the prediction is modified using the partition function obtained by including the \textbf{fluctuations} up to second order, where again we now include the regular part \( Z_0 \) of the partition function, the saddle-point contribution, and the fluctuation contribution:
          \[
              -\log Z = -\log Z_0 + V\left( \frac{t}{2}\overline{m}^2 + u \overline{m}^4 \right) + \frac{V}{2} \int \frac{\mathrm{d}^d \mathbf{q}}{(2\pi)^d} \log(c_2 q^2 + c_1),
          \]
          where we had
          \[
          ...
          \begin{itemize}
              ...
              \item For \(d \geq 4\) the integral diverges at large \(q\), and is dominated by the momentum cutoff \(\Lambda\): we would have an integrand behaving as \(q^{-4}\) at large \(q\), and thus the integral would diverge as \(\Lambda^{d-4}\).
...
The same machinery can be used to compute \uline{correlation functions} and in particular the connected correlator
\[
...
\[
    \implies \quad |t|^{\frac{d-4}{2}} \ll \frac{\kappa}{u} \qquad \Rightarrow \qquad |t| \gg t_G \simeq \left( \frac{u}{\kappa} \right)^{\frac{2}{4-d}}.
\]
