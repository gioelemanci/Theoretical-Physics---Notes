...
\begin{example}
    ...
    The function \(\overline{v}_x(\{q_j\},\,\{p_j\})\) is defined on the phase space of the system, and it assigns a value to each microstate of the system. Given the knowledge on some initial microstate, the system will evolve along a curve in the fixed energy shell defined by \(E\):
    \[
...
\]
where \(\Gamma_k(E) = \int_{E < H_k < E + \Delta} \mathrm{d}\{q_{kf}\} \mathrm{d}\{p_{kf}\}\) is the number of states available to the \(k\)-th subsystem at energy \(E\).
...
\begin{itemize}
    \item Postulate of equal a priori probabilities: in an isolated system, all microstates are equally probable,
          \[
              \vert \bar{c_n} \vert^2 = \begin{dcases}
