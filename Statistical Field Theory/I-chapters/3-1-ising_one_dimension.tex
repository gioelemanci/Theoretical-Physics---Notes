...
\[
    ...
    \end{pmatrix},
\]
with the idea of diagonalizing this matrix to find its eigenvalues \(\lambda_1\) and \(\lambda_2\). Let us note that \(T = T^{\dagger}\), thus we know there exist an orthogonal matrix \(O^{-1} = O^T\) such that \(T = O D O^{T}\), where \(D\) is a diagonal matrix containing the eigenvalues of \(T\). The diagonal maatrix \(D\) can be expressed as
\[
    D = \begin{pmatrix}
        \lambda_+ & 0         \\
        0         & \lambda_-
