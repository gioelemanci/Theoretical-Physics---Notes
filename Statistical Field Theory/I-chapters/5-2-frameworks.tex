...
\begin{enumerate}
    \item \textbf{Coarse-Grain}: we ``decrease the resolution'' of each configuration, changing the description of the system defining a new order parameter field \(m^{\prime}(\mathbf{x}^{\prime})\) as the average of the original order parameter field \(m(\mathbf{x})\) over mesoscopic regions of some lenght scale \(b\):
          \[
              m^{\prime}(\mathbf{x}) = \frac{1}{b^d} \int_{\mathcal{C}_b(\mathbf{x})} m(\mathbf{x}^{\prime}) \mathrm{d} \mathbf{x}^{\prime},
          \]
...
\[
    m^{*}(\mathbf{x}^{*}) = \frac{1}{\zeta b^d} \int_{\mathcal{C}} \mathrm{d}^d m(\mathbf{x}^{\prime}),
\]
...
\[
    \begin{aligned}
        F[m] & = \int \mathrm{d}^d \mathbf{x} \left[ \frac{t^*}{2} m^2 + u^* m^4 + v^*_{1} m^6 + v^*_{2} m^8 \right.                       \\
             & \left. + \cdots + \frac{k^*}{2} (\nabla m)^2 + \frac{L^*}{2} (\nabla m)^2 + \cdots + h^* m + h^* m^{5 / 7}+ \cdots \right],
    \end{aligned}
\]
...
\]
where we have denoted the Jacobian matrix of the map \(\mathcal{R}_b\) at the fixed point as \( \mathcal{R}_b^L = \left. \frac{\partial \mathcal{R}_b}{\partial \mathbf{s}} \right|_{\overline{\mathbf{s}}}\).
...
\[
    \mathcal{R}_b \mathcal{R}_{b^{\prime}} O_i = \lambda_i(b)\lambda_i(b^{\prime}) = \mathcal{R}_{bb^{\prime}} O_i = \lambda_i(bb^{\prime}) O_i,
\]
...
\[
    ...
    \end{dcases}
\]
Since the eigenvectors \(O_i\) form a complete basis, we can write any Hamiltonian in the vicinity of \(\overline{\mathbf{s}}\) as
\[
