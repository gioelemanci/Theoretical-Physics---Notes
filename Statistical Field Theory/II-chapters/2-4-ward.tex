...
\[
    \Delta_T = 2 \;, \quad s_T = 2
\]
\[
    \Delta_{\bar{T}} = 2 \;, \quad s_{\bar{T}} = -2
\]
...
The full OPE of \(T(z)T(w)\) for \(z \to w\) is tightly constrained. Both scale invariance and the requirement that correlation functions of local fields must be meromorphic dictate the general structure of the singular terms:
\[
    T(z)T(w) = \frac{c/2}{(z-w)^4} + \frac{J(w)}{(z-w)^3} + \frac{Q(w)}{(z-w)^2} + \frac{R(w)}{z-w} + \text{regular terms}
\]
where the generic operators must have specific holomorphic dimensions: \(h_J = 1\), \(h_Q = 2\), and \(h_R = 3\).
...
\]
Assuming analyticity of the fields, we can express the right-hand side entirely at the coordinate \(w\) by Taylor expanding the fields \(J(z)\) and \(Q(z)\) around \(w\). For instance:
\[
...
\]
If we now make the highly reasonable physical hypothesis that \(T(z)\) is the \textit{only} conformal field of spin and dimension 2 in the algebra of local fields, then \(Q(w)\) must be proportional to \(T(w)\) itself. We can write \(Q(w) = \kappa T(w)\). The OPE thus simplifies to:
\[
    T(z)T(w) = \frac{c/2}{(z-w)^4} + \frac{\kappa T(w)}{(z-w)^2} + \frac{\frac{\kappa}{2} \partial_w T(w)}{z-w} + \text{regular terms}
\]
By applying a global dilatation \(z, w \to \lambda z, \lambda w\) and requiring dimensional consistency, we can easily fix the proportionality constant to \(\kappa = 2\). This yields the fundamental \(TT\) operator product expansion valid for any two-dimensional Conformal Field Theory:
\[
...
We also recall that the infinitesimal variation of any local conformal field \(A(z, \bar{z})\) under a generic holomorphic conformal transformation \(\epsilon(z)\) is generated by its commutator with the conserved charge, which can be expressed as a contour integral of the OPE:
\[
...
\begin{example}[Exercise: Infinitesimal Limit of the Schwarzian Transformation]
