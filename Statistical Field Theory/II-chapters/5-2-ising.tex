...
\[
    :\sigma^2(w, \bar{w}): \;\equiv \lim_{z,\bar{z} \to w} |z-w|^\delta \left( \sigma(z,\bar{z})\sigma(w,\bar{w}) - \text{singular part} \right).
\]
In our specific case, the singular part is \(C_{\sigma\sigma}^{\mathbb{I}} |z-w|^{-1/4}\). Once subtracted, we are left with \(C_{\sigma\sigma}^\varepsilon |z-w|^{3/4} \varepsilon(w, \bar{w})\). To exactly cancel the fractional branch and ensure a single-valued, well-defined right-hand side, we must elegantly choose \(\delta = -3/4\). The limit then immediately yields:
\[
    :\sigma^2(w, \bar{w}): \; = \varepsilon(w, \bar{w}).
\]
...
We can proceed analogously to mathematically define \(:\sigma^3(w, \bar{w}):\) by considering the OPE of \(\sigma\) with \(:\sigma^2:\) (which is \(\varepsilon\)). We subtract the singular term \(|z-w|^{-1}\), compensate for the branch cut of the next leading term, and take the limit. The remaining field is precisely the scalar descendant we noted earlier:
\[
    \square_w \sigma = g :\sigma^3(w, \bar{w}):
\]
where \(\square_w = \partial_w \bar{\partial}_w\) is the d'Alembertian/Laplacian in complex coordinates, and \(g = (\beta C_{\sigma\sigma}^\varepsilon)^{-1}\).
...
\end{itemize}
Substituting these into the formula, the exact three-point function is:\TODO{i think exponents are wrong}
\[
...
Finally, to cleanly encode the entire spectrum of states residing within each of these conformal families, we compute their characters. For the Ising model, there are only three such functions:
\[
