...
\begin{itemize}
    ...
    \item The coefficients \(a_N, \bar{a}_{\bar{N}}\) in the Taylor expansion of the characters give the dimensionalities of the secondary descendant levels.
\end{itemize}
...
\[
    Z(q, \bar{q}) = (q \bar{q})^{-c/24} \sum_{h, \bar{h}} \mathcal{N}_{h,\bar{h}} q^h \bar{q}^{\bar{h}} \sum_{N, \bar{N}=0}^\infty a_N q^N \cdot \bar{a}_{\bar{N}} \bar{q}^{\bar{N}}.
\]
...
\]
where \(\tilde{G}(p) = \int_{-\infty}^{+\infty} G(x)e^{-ipx}dx\). A heuristic physics proof (allowing for the use of distributions) relies on the Dirac comb identity in distribution theory: $\sum \delta(x - nT) = \frac{1}{T} \sum e^{2\pi i k x / T}$.
...
\[
    Z\left(-\frac{1}{\tau}, -\frac{1}{\bar{\tau}}\right) = \sum_{h, \bar{h}} \mathcal{N}_{h,\bar{h}} \left( \sum_{h'} \mathcal{S}_{h, h'} \chi_{h'}(\tau) \right) \left( \sum_{\bar{h}'} \mathcal{S}_{\bar{h}, \bar{h}'}^* \bar{\chi}_{\bar{h}'}(\bar{\tau}) \right).
\]
...
\[
    Z_{1,g}(\tau, \bar{\tau}) = \mathrm{Tr}_{\mathcal{H}} \left[ \hat{g} q^{L_0 - \frac{c}{24}} \bar{q}^{\bar{L}_0 - \frac{c}{24}} \right] = \sum_{\Delta, \bar{\Delta}} \mathcal{N}_{\Delta, \bar{\Delta}} \, g \, \chi_{\Delta}(\tau) \bar{\chi}_{\bar{\Delta}}(\bar{\tau}).
\]
...
\begin{example}[Twisted Sectors in the Ising Model]
    ...
    \[
        Z_{+,-} = \sum \mathcal{N}_{\dots} P_{r,s} \chi_{r,s} \bar{\chi}_{r,s}, \quad \text{where } P_{r,s} = \begin{cases} (-1)^r & m \text{ even} \\ (-1)^s & m \text{ odd} \end{cases}
    \]
