...
\begin{enumerate}
    \item If \( \mathbf{k} + \mathbf{q} = 0 \) and \( \mathbf{k}' - \mathbf{q} = 0 \), we have that \( \mathbf{k} = -\mathbf{q} \) and \( \mathbf{k}' = \mathbf{q} \), and so we remain with
          ...
          At this stage, we introduce a crucial physical approximation to replace the interaction potential \( U(\mathbf{q}) \) with a simpler constant, \( U(0) \). If we assume that the potential is sufficiently smooth, monotonic, and effectively short-ranged, its Fourier transform will be approximately constant for small momenta. Consequently, the main contribution to the interaction term comes from the zero-momentum component of the potential. In the context of two-body interactions, van der Waals forces serve as an excellent example of a short-ranged potential. In the standard Lennard-Jones potential, the attractive part is proportional to \( 1/r^6 \) and the repulsive part to \( 1/r^{12} \). We could also consider dipole interactions, which scale as \( 1/r^3 \); this represents a tricky marginal case for the short-range assumption. However, since we are primarily dealing with dilute atomic gases, van der Waals interactions are effectively short-ranged because the typical interparticle distance is much larger than the atomic radius. With this assumption, we can set \( U(\mathbf{q}) \approx U(\mathbf{q}=0) \equiv U_0 \), making calculations significantly simpler. The first term thus becomes:\TODO{change: before the first bullet point, we should introduce the short-range assumption and explain why it is reasonable, and the result is that it has to decay faster than 1 over r cube.}
          \[
...
\]
\TODO{add summary of assumptions: massive, weakly interacting, short-range, etc.}
...
\subsubsection{Non Interacting Limit}

In the limit of vanishing interactions, \( U_0 \to 0 \), the Bogoliubov spectrum reduces to the free-particle dispersion: ...\TODO{add E and Eps in non interacting limit.}
