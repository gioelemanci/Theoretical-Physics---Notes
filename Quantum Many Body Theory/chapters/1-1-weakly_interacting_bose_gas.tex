...
\TODO{Add a figure of the box with periodic boundary conditions}
...
Besides the dominant term where all operators are in the condensate state, we retain six terms from the interaction Hamiltonian that contain exactly two operators with momentum different from zero:
...
\begin{enumerate}
    \item If \( \mathbf{k} + \mathbf{q} = 0 \) and \( \mathbf{k}' - \mathbf{q} = 0 \), we have that \( \mathbf{k} = -\mathbf{q} \) and \( \mathbf{k}' = \mathbf{q} \), and so we remain with
          \[
              \frac{1}{2V} a_0^\dagger a_0^\dagger \sum_{\mathbf{q}}' U(\mathbf{q}) a_{\mathbf{q}} a_{-\mathbf{q}},
          \]
          ...
          At this stage, we introduce a crucial physical approximation to replace the interaction potential \( U(\mathbf{q}) \) with a simpler constant, \( U(0) \). If we assume that the potential is sufficiently smooth, monotonic, and effectively short-ranged, its Fourier transform will be approximately constant for small momenta. Consequently, the main contribution to the interaction term comes from the zero-momentum component of the potential. In the context of two-body interactions, van der Waals forces serve as an excellent example of a short-ranged potential. In the standard Lennard-Jones potential, the attractive part is proportional to \( 1/r^6 \) and the repulsive part to \( 1/r^{12} \). We could also consider dipole interactions, which scale as \( 1/r^3 \); this represents a tricky marginal case for the short-range assumption. However, since we are primarily dealing with dilute atomic gases, van der Waals interactions are effectively short-ranged because the typical interparticle distance is much larger than the atomic radius. With this assumption, we can set \( U(\mathbf{q}) \approx U(\mathbf{q}=0) \equiv U_0 \), making calculations significantly simpler. The first term thus becomes:
          \[
              \frac{1}{2V} U_0 a_0^\dagger a_0^\dagger \sum_{\mathbf{q}}' a_{\mathbf{q}} a_{-\mathbf{q}}.
          \]

          \begin{remark}
              As an analogous example of effective short-range interactions from high-energy physics, the strong nuclear force is also effectively short-ranged because its mediators, pions, are massive.
          \end{remark}
...
By substituting these numerical expectations back into our aggregated interaction terms, the full Hamiltonian becomes:
...
\end{equation}
for \( \mathbf{k} \neq 0 \), where \( u_k \) and \( v_k \) are real coefficients. These coefficients are defined to depend strictly on the momentum magnitude \( k = |\mathbf{k}| \), ensuring that rotational invariance is preserved in momentum space.
...
\]
where \( n_{\mathbf{k}_i} = \alpha_{\mathbf{k}_i}^\dagger \alpha_{\mathbf{k}_i} \) is the occupation number of the \( i \)-th state in the Bogoliubov quasiparticle basis.

It is critical to note that the vacuum of these newly defined operators, \( |\psi_0\rangle \), is distinct from the vacuum of the original bare particle operators, \( a_{\mathbf{k}} \). This distinction becomes physically apparent when we evaluate the expectation value of the bare particle number operator in the interacting ground state:
\[
