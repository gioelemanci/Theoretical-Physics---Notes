\section*{Sheet 5}
\addcontentsline{toc}{section}{Sheet 5}


\subsection*{1. Proof of the Gordon Identity}
\addcontentsline{toc}{subsection}{1. Proof of the Gordon Identity}
...
\addcontentsline{toc}{subsection}{2. Vacuum polarization and renormalization in QED}

\begin{center}
    \textit{We showed in the lectures that the full photon propagator in QED can be written as}
    \[
        \frac{-i g_{\mu\nu}}{p^2 (1 - \Pi(p^2))} + \mathcal{O}(p^\mu p^\nu), \qquad (3)
    \]
    \textit{where} $\mathcal{O}(p^\mu p^\nu)$ \textit{stands for terms that will vanish once contracted with an electromagnetic current, due to the Ward identity. The term} $\Pi(p^2)$ \textit{is instead defined by}
    \[
        i \Pi^{\mu\nu} = i (p^2 g^{\mu\nu} - p^\mu p^\nu) \Pi(p^2), \qquad (4)
    \]
    \textit{where the left-hand side is the sum of all the one-particle irreducible (1PI) diagrams contributing to the propagator.}

    \textit{Compute the renormalized } $\Pi(p^2)$\textit{, using the following renormalization schemes}
    \begin{enumerate}
        \item \textit{on-shell scheme, given by the renormalization condition} $\Pi(0) = 0$,
        \item \textit{in the} $\overline{MS}$ \textit{scheme}.
    \end{enumerate}
\end{center}

\textbf{One-Loop Vacuum Polarization Tensor}
...
\textbf{Feynman Parameterization and Loop Integration}
...
\addcontentsline{toc}{subsection}{3. Computation of the QED $\beta$ function at one-loop order}
...
\addcontentsline{toc}{subsection}{4. One-loop corrections to the photon propagator in scalar QED}
...
\addcontentsline{toc}{subsection}{5. Fermion mass renormalization constant in the MS scheme}
...
\addcontentsline{toc}{subsection}{6. Optional: One-loop $\beta$ function in Yukawa theory}
...
\addcontentsline{toc}{subsection}{7. Optional: One-loop $\beta$ functions for $\lambda_3$ and $\lambda_4$ in Yukawa theory}
...
\addcontentsline{toc}{subsection}{8. Optional: Explicit computation of $Z_1$, $Z_2$, and $Z_m$ in QED (On-Shell Scheme)}
