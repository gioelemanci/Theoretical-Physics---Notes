...
\[
    ...
    \end{aligned}
\]
Paying again attention to the \((2\pi)\) factors, we can perform the integrals over the external points \(x_1, x_2, x_3, x_4\), which gives us delta functions that set the momenta of the internal lines to be equal to the momenta of the external lines, enforcing momentum conservation at each vertex. Thus simplifing a factor of \((2\pi)^4\) for each delta function, we get a term of the form
\[
...
\]
where the first four delta functions set the internal momenta to be equal to the external momenta, while the last delta function enforces momentum conservation at the vertices. Thus in the end we can perform the integrals over the internal momenta \(k_1, k_2, k_3, k_4\), enforcing \(k_j = p_j\) for \(j=1,2,3,4\), which leads us to
\[
