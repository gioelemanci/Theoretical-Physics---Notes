...
\]
If \(\mathcal{L}_{\text{int}} \) does indeed contain derivatives, then similar results can be obtained, but it will be easier to derive them in the path integral formalism, which is more convenient for theories with derivative interactions, and we will see how to do that in the next chapters.

\subsection{Interaction Picture}

We are now moving from the Heisenberg picture to the interaction picture (also called the Dirac picture), where both operators and states evolve in time. In the interaction picture, the \textit{fields evolve according to the free Hamiltonian} \(H_0\), while the \textit{states evolve according to the total Hamiltonian} \(H = H_0 + V\): the interaction is encoded in the evolution of the states, while the fields evolve as if they were free fields. This is a very important point, as it allows us to use the free fields to calculate the interactions, which is much easier than dealing with the full interacting fields.

Then of course the \textit{matrix elements} we are interested in computing \textit{will be independent from the chosen picture}, since they are physical quantities, and thus we can choose the picture that is more convenient for our calculations. Schrödinger, Heisenberg and Dirac picture are just different ways of describing the same physics, and they are identically equivalent equivalent at a conventional reference time \(t = t_0\).
...
\begin{equation}
    ...
    \label{eq:interaction-picture-evolution}
\end{equation}
with
\[
...
The solution to the evolution equation for \(\hat{U}\) \eqref{eq:interaction-picture-evolution} can be written as the following well-known time-ordered exponential:
\[
...
\]
where \(\ket{\Omega}\) is the vacuum state of the interacting theory, and \(\hat{\phi} (x) = \hat{\phi}_H (x)\) are the interacting fields in the Heisenberg picture.

To evaluate this, we need to relate the interacting vacuum \(\ket{\Omega}\) to the free theory vacuum \(\ket{0}\). We assume that at the infinite past (\(t = -\infty\)), the interacting vacuum behaves like the free vacuum, meaning it is annihilated by the Heisenberg annihilation operator \(a_{\mathbf{p}}(t)\):
\[
    a_{\mathbf{p}}(t) \ket{\Omega} = 0 \quad \text{at } t = -\infty.
\]
Using the time evolution operator \(S(t, t_0)\), we can rewrite the annihilation operator as \(a_{\mathbf{p}}(t) = S^\dagger(t, t_0) a_{\mathbf{p}}(t_0) S(t, t_0)\). This implies that \(a_{\mathbf{p}}(t_0)\) annihilates the state \(S(t, t_0) \ket{\Omega}\) at \(t = -\infty\).

At the reference time \(t = t_0\), the interaction and Heisenberg pictures coincide. Thus, \(a_{\mathbf{p}}(t_0)\) is identical to the annihilation operator of the free theory, which naturally annihilates the free vacuum: \(a_{\mathbf{p}}(t_0) \ket{0} = 0\).
Furthermore, the free vacuum is an eigenstate of the free Hamiltonian \(\hat{H}_0\). Assuming the vacuum energy is zero (\(E_0 = 0\)), we have \(e^{i \hat{H}_0 (t - t_0)} \ket{0} = \ket{0}\), which means:
\[
    a_{\mathbf{p}}(t_0) e^{i \hat{H}_0 (t - t_0)} \ket{0} = 0.
\]
Because both \(S(t, t_0)\ket{\Omega}\big|_{t = -\infty}\) and \(e^{i \hat{H}_0 (t - t_0)} \ket{0}\) are annihilated by the exact same operator \(a_{\mathbf{p}}(t_0)\), the two states must be proportional. We can express this proportionality using the standard interaction picture time-evolution operator \(U(t_f, t_i)\):
\[
    \ket{\Omega} = N_i U(0, -\infty) \ket{0},
\]
for some normalization constant \(N_i\). By an analogous argument for the bra vector at \(t = +\infty\), we find:
\[
    \bra{\Omega} = N_f \bra{0} U(+\infty, 0).
\]

We can now evaluate the Green's function. Let us assume, up to a relabelling of the coordinates \(x_i = (t_i, \mathbf{x}_i)\), that the times are strictly ordered: \(t_1 > t_2 > \dots > t_n\). Because of this explicit ordering, we can temporarily drop the time-ordering operator \(\hat{T}\). We also express the Heisenberg fields in terms of the free interaction-picture fields \(\hat{\phi}_0\) using \(\hat{\phi}(x_i) = U(0, t_i) \hat{\phi}_0(x_i) U(t_i, 0)\). Substituting these elements yields:
\[
    \begin{aligned}
        \bra{\Omega} \hat{T} \{ \hat{\phi}(x_1) \dots \hat{\phi}(x_n) \} \ket{\Omega}
        = \bra{\Omega} \hat{\phi}(x_1) \dots \hat{\phi}(x_n) \ket{\Omega} \\
        = N_i N_f \bra{0} U(+\infty, 0) \left( U(0, t_1) \hat{\phi}_0(x_1) U(t_1, 0) \right) \dots \left( U(0, t_n) \hat{\phi}_0(x_n) U(t_n, 0) \right) U(0, -\infty) \ket{0}.
    \end{aligned}
\]
Using the composition property of the evolution operator (\(U(t_a, t_b) U(t_b, t_c) = U(t_a, t_c)\)), all the intermediate adjacent operators combine gracefully:
\[
    = N_i N_f \bra{0} U(+\infty, t_1) \hat{\phi}_0(x_1) U(t_1, t_2) \hat{\phi}_0(x_2) \dots \hat{\phi}_0(x_n) U(t_n, -\infty) \ket{0}.
\]
At this point, every operator inside the expectation value \(\bra{0} \dots \ket{0}\) is time-ordered by construction (since \(U(t_a, t_b)\) already contains time-ordered products of the interaction potential \(\hat{V}_I(t)\) with \(t_b \leq t \leq t_a\)). We can therefore safely put the global time-ordering operator \(\hat{T}\) back in. Since operators commute inside \(\hat{T}\), we can group all the independent evolution operators together into one:
\[
    \begin{aligned}
        \bra{\Omega} \hat{T} \{ \hat{\phi}(x_1) \dots \hat{\phi}(x_n) \} \ket{\Omega}
         & = N_i N_f \bra{0} \hat{T} \{ \hat{\phi}_0(x_1) \dots \hat{\phi}_0(x_n) U(+\infty, t_1) U(t_1, t_2) \dots U(t_n, -\infty) \} \ket{0} \\
         & = N_i N_f \bra{0} \hat{T} \{ \hat{\phi}_0(x_1) \dots \hat{\phi}_0(x_n) U(+\infty, -\infty) \} \ket{0}.
    \end{aligned}
\]
Finally, we fix the overall constant \(N_i N_f\) by enforcing the standard normalization condition for the vacuum, \(\langle\Omega|\Omega\rangle = \langle 0|0 \rangle = 1\). Setting \(n=0\) (no fields) in our derivation above gives \(\langle \Omega | \Omega \rangle = N_i N_f \bra{0} \hat{T} \{ U(+\infty, -\infty) \} \ket{0}\), which implies:
\[
    N_i N_f = \frac{1}{\bra{0} \hat{T} \{ U(+\infty, -\infty) \} \ket{0}}.
\]
Recalling that \(U(+\infty, -\infty) = \exp\left[ -i \int_{-\infty}^{+\infty} \mathrm{d}t \, \hat{V}_I(t) \right]\), we obtain the final formula (also known as the \textit{Gell-Mann and Low} theorem):
\[
    \bra{\Omega} \hat{T} \{ \hat{\phi} (x_1) \dots \hat{\phi} (x_n) \} \ket{\Omega} = \frac{\bra{0} \hat{T} \{ \hat{\phi}_0 (x_1) \dots \hat{\phi}_0(x_n) \exp[ -i \int_{-\infty}^{\infty} \mathrm{d} t \, \hat{V}_I(t) ] \}\ket{0}}{\bra{0} \hat{T} \{ \exp[ -i \int_{-\infty}^{\infty} \mathrm{d} t \, \hat{V}_I(t) ] \} \ket{0}}.
\]
The right-hand side is now entirely expressed in terms of free fields \(\hat{\phi}_0\) and the free vacuum \(\ket{0}\), objects we know how to deal with using Wick's theorem and standard perturbation theory.
...
\]
where \(\hat{\phi} (x)\) is a free scalar field operator (we are dropping the subscript 0 for simplicity), thus behaving as a ladder operator. We can manipulate the fields as
\[
...
\begin{theorem}[Wick's theorem]
    The time-ordered product of \(n\) fields can be written as the normal-ordered product of those \(n\) fields plus the sum of all normal-ordered terms containing one, two, or any number of simultaneous pairwise contractions between those fields:
    \[
...
\begin{corollary}
    ...
    \[
        \bra{0} \hat{T} \{ \hat{\phi} (x_1) \dots \hat{\phi} (x_n) \} \ket{0} = \text{all possible } \textbf{fully contracted} \text{ terms},
    \]
    since non-fully contracted terms will be multiplied by a normal ordered product (of the non-contracted fields) which vacuum expectation value is zero.
\end{corollary}
This is a very important result, as it allows us to calculate Green's functions in terms of Feynman propagators, which are much easier to calculate than the full time-ordered product of fields.
\begin{corollary}
    If in a free theory we have an odd number of fields, then there are no possible fully contracted terms, since we cannot fully contract an odd number of fields, thus we have
    \[
        \bra{0} \hat{T} \{ \hat{\phi} (x_1) \dots \hat{\phi} (x_{2n+1}) \} \ket{0} = 0.
    \]
\end{corollary}
This is very important, as it tells us that the vacuum expectation value of the time-ordered product of an odd number of fields is always zero (there will always be at least one field that is not contracted, and thus there will remain a normal-ordered product, whose vacuum expectation value is zero). Fundamentally, this is a consequence of the exact \(\mathbb{Z}_2\) symmetry (\(\hat{\phi} \to -\hat{\phi}\)) of the free Lagrangian, which forces any odd-point correlation function to vanish.

For an interacting theory, this is not necessarily true, but it depends strictly on the symmetries of the interaction. If the interacting Lagrangian preserves the \(\mathbb{Z}_2\) symmetry (like in a \(\phi^4\) theory), the vacuum expectation values of an odd number of fields will still remain zero. However, if the interaction explicitly breaks this symmetry (e.g., a \(\phi^3\) interaction) or if the symmetry is spontaneously broken (as in the Higgs mechanism, where \(\bra{\Omega}\hat{\phi}\ket{\Omega} \neq 0\)), then odd-point Green's functions can acquire non-zero values.
...
\begin{example}[\(\phi^3\) theory]
    ...
    \]
    where \(\ket{\Omega}\) is the vacuum state of the interacting theory and \(\hat{\phi}\) are the field operators. Using the formula we derived before for the Green's function in the interaction picture, we can write
    \[
    ...
    \uline{The first term in the expansion (order \(\mathcal{O}(\lambda^0)\)) corresponds to the free theory}, which is just a single line connecting the two external points \(x_1\) and \(x_2\), representing the propagation of a free particle.
    \uline{The second term (order \(\mathcal{O}(\lambda)\)) strictly vanishes} by Wick's theorem, since it contains an odd number of fields (five in total: two external and three from the interaction vertex), making it impossible to fully contract them.
    \uline{The third term (order \(\mathcal{O}(\lambda^2)\)) is the first non-vanishing correction}, containing eight fields. Its connected part corresponds to a \textbf{diagram with a single loop}, where the particle propagates from \(x_1\) to an interaction vertex \(x\), splits into two virtual particles that propagate to another vertex \(y\) (forming the loop), and then recombines to propagate to \(x_2\).
    ...
    As we have highlighted, due to Wick's theorem, all corrections of odd order in \(\lambda\) vanish for the 2-point function in \(\phi^3\) theory. The physical corrections start at \(\mathcal{O}(\lambda^2)\) with a one-loop diagram, and higher-order terms will progressively include more complex multi-loop topologies.
\end{example}

\subsubsection{Shortcut Notation}
...
\[
    (-\frac{i \lambda}{3!})^2 \frac{1}{2!} \int \mathrm{d}^4 x \, \mathrm{d}^4 y \, \bra{0} \hat{T} \{ \hat{\phi}_{x_1} \hat{\phi}_{x_2} \hat{\phi}_{x} \hat{\phi}_{x} \hat{\phi}_{x} \hat{\phi}_{y} \hat{\phi}_{y} \hat{\phi}_{y}\} \ket{0},
\]
...
Now we can start to derive the Feynman rules for this theory, which will allow us to write down the contribution of each diagram without having to calculate all the contractions explicitly. The Feynman graphical representation can be derived from the previous example, where we had the following elements:
...
\begin{example}[Back to the cubic interaction]
    ...
    \[
        \frac{1}{2} \times (-i \lambda)^2 \int \mathrm{d}^4 z_1 \, \mathrm{d}^4 z_2 \, D_{x_1 x} D_{x_2 y} D_{xy}^2,
    \]
...
\[
    \bra{\Omega} \hat{T} \{ \hat{\phi} (x_1) \hat{\phi} (x_2) \cdots \hat{\phi} (x_n) \} \ket{\Omega} = \sum \text{connected diagrams}.
\]

\begin{example}[Still on the cubic interaction]
    If we were to show the previous result explicitly for the \(\phi^3\) theory, we would have to compute the green's function up to order \(O(\lambda^4)\), which would give us the following diagrams:\TODO{vacuum diagrams do not have the free prop, remove it}
    \[
        \begin{gathered}
...
\subsubsection{Summary of Feynman Rules in Coordinate Space}
