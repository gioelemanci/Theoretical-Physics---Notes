...
\[
    ...
    \end{aligned}
\]
where we have used the LSZ reduction formula to relate the S-matrix element to the Green's function. Now for a free theory \(Z=1\), while for an interacting theory \(Z = 1 + O(\lambda)\), thus at the leading order we can set \(Z=1\) (since we have lower order terms). Now we can use the Feynman rules in position space to write down the contribution of the leading order diagram, given by
\[
    \begin{aligned}
...
\]
Following this procedure for the first diagram, we get
\[
    \begin{aligned}
...
Finishing the computation for the leading order correction, we had the three channel diagrams:
\[
    \mathcal{O}(\lambda^2) = \feynFourS + \feynFourT + \feynFourU,
\]
where the contribution of the deltas in the internal lines (internal momenta) is respectively given by
\begin{itemize}
...
\[
    \bra{\Omega} \hat{T} \{ \hat{\phi}(x_1) \dots \hat{\phi}(x_n) \hat{\phi}(x_a) \hat{\phi}(x_b) \} \ket{\Omega}.
\]
