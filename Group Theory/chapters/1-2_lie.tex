\section{Lie Groups and Lie Algebras} \label{sec:lie}
...
\[
    \bs{\theta}'' = \mathbf{m}(\bs{\theta}', \bs{\theta}),
\]
which is a function of the \textbf{group multiplication}. The function \(\mathbf{m}\) is smooth as the group multiplication is smooth. The inverse of \(g(\bs{\theta})\) is given by
\[
    g(\bs{\theta})^{-1} = g(\bs{\theta}'),
\]
for some \(\bs{\theta}' = \mathbf{j}(\bs{\theta})\), which is unique since the parameterization is one-to-one. The function \(\mathbf{j}\), implementing the group inversion, is smooth as the group inversion is smooth.

We have thus found two smooth functions \(\mathbf{m}\), \(\mathbf{j}\) that encode the group multiplication and inversion in terms of the parameters \(\bs{\theta}\):
\[
    \begin{dcases}
        \mathbf{m} : D \times D \to D, \\
        \mathbf{j} : D \to D,
    \end{dcases} \quad \text{such that} \quad \begin{dcases}
        g(\bs{\theta}') g(\bs{\theta}) = g(\mathbf{m}(\bs{\theta}', \bs{\theta})), \\
        g(\mathbf{j}(\bs{\theta})) = g(\bs{\theta})^{-1}.
    \end{dcases}
\]

These functions have to respect some properties, which are the group multiplication properties: associativity, invertibility and neutrality. Thus we can express the next proposition.
...
\begin{proposition}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    Given any choice of three parameter vectors \(\bs{\theta}\), \(\bs{\theta}'\), \(\bs{\theta}''\). These identities constrain the form of the group multiplication function \(m\) and the inversion function \(j\) to an important extent.
\end{proposition}

\begin{proof}\TODO{Complete proof}
...
The idea now is to understand the structure of \(G\) by looking at the behavior of \(g(\bs{\theta})\) in a neighborhood of the neutral element. This is done by Taylor expanding in power series around \(\mathbf{0}\):
\[
...
\[
    g(\mathbf{0}) = 1, \quad t_a = \frac{\partial g}{\partial \theta^a} \bigg|_{\mathbf{0}}, \quad t_{ab} = \frac{\partial^2 g}{\partial \theta^a \partial \theta^b} \bigg|_{\mathbf{0}},
\]
so that
\[
