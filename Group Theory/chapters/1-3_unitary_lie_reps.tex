...
\begin{itemize}
    \item \(\hat{\tilde{U}}(\tilde{\mathbf{j}}(\bs{\theta})) = \hat{\tilde{U}}(\bs{\theta})^{-1}\)
    \item \(\hat{\tilde{U}}(\tilde{\mathbf{m}}(\bs{\theta}, \bs{\theta'})) = \hat{\tilde{U}}(\bs{\theta}) \hat{\tilde{U}}(\bs{\theta'})\)
\end{itemize}
...
\begin{example}[The proper rotation group \(\mathrm{SO(3)} / \mathrm{SU}(2)\)]
    ...
    \]
    where \(D(\bs{\varphi})\) is the exponential parametrization of the representation of the rotation group \(SO(3)\) on the internal degrees of freedom of the quantum system. We have to look at the expansion of the unitary operators \(U(\bs{\varphi})\) for small rotations \(\delta \bs{\varphi}\) to get the generators \(\mathbf{J}\) of the representation \(U\) of the proper rotation group \(SO(3)\):
    \[
        U(\delta \bs{\varphi}) = 1 - i \mathbf{J} \cdot \delta \bs{\varphi} + \mathrm{O}(\delta \varphi^2),
    \]
    where \(\mathbf{J} = (J_1, J_2, J_3)\) are the generators of the representation \(U\) of the proper rotation group \(SO(3)\), which are called the angular momentum operators of the quantum system. Then the expansion of the internal degrees transformation \(D(\bs{\varphi})\) for small rotations \(\delta \bs{\varphi}\) gives us
    \[
    ...
    Thus we found
    \[
        \mathbf{J} = \hbar \bs{\Sigma} - i\hbar \mathbf{x} \times \nabla,
    \]
    where renaming \(\hbar \bs{\Sigma} = \mathbf{S}\) and \(-i \hbar \mathbf{x} \times \nabla = \mathbf{L}\) we get the familiar decomposition of the angular momentum operator \(\mathbf{J}\) into the spin operator \(\mathbf{S}\) and the orbital angular momentum operator \(\mathbf{L}\):
    \[
    ...
    \[
        U(\bs{\varphi}) = e^{-\tfrac{i}{\hbar} \bs{\varphi} \cdot \mathbf{J}}.
    \]
    \begin{proof}
        limit for n....
    \end{proof}
