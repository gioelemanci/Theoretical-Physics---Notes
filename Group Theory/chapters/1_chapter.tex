...
\[
    g(\mathbf{0}) = 1,
\]
a convenient convention. By virtue of the parametrization, for any pair of parameter vectors \(\bs{\theta}\), \(\bs{\theta}'\), there exists a third parameter vector \(\bs{\theta}''\) such that
\[
...
\]
for some \(\bs{\theta}' = j(\bs{\theta})\), which is unique since the parameterization is one-to-one. The function \(j\) is smooth as the group inversion is smooth.
...
Thus the tangent space to \(G\) at the identity is a vector space of dimension \(d\) spanned by the matrices \(t_a\), \(a = 1, \ldots, d\), costituing a basis. We denote this tangent space by \(\mathfrak{g}\) and call it the Lie algebra of \(G\).
...
\]
we can note how the coefficients \(t_a\) define another member of the Lie algebra \(\mathfrak{g}\) by the commutator
\[
